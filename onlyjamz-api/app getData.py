...
def chatgpt():
    if request.method == "POST":
        ...
        getData = submittedRequest['getData']
        response = openai.Completion.create(
            model="text-davinci-003",
            max_tokens=2048,
            prompt=generate_prompt(getData),
            temperature=0.6,
        )
...
def generate_prompt(getData):
    return """Suggest 5 indie game ideas for a game jam, each idea should be 1 paragraph long. The Game should be about {}""".format(getData)
